                ...
                <div style="display: flex; gap: 1rem; padding: 1rem; border: 1px solid #eee; margin-bottom: 1rem; border-radius: 8px;">
                    <div style="width: 100px; height: 100px;">
                        @if($item->product->image_url)
                            @php $src = preg_match('/^https?:\\/\\//i', $item->product->image_url) ? $item->product->image_url : Storage::url($item->product->image_url); @endphp
                            <img src="{{ $src }}" alt="{{ $item->product->name }}" style="width:100%; height:100%; object-fit:cover;">
                        @else
                            <div style="width:100%; height:100%; background: var(--light-beige); display: flex; align-items: center; justify-content: center;">📦</div>
                        @endif
                    </div>
                    <div style="flex: 1;">
