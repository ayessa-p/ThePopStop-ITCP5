...
<div class="admin-container">
    <aside class="admin-sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ route('admin.suppliers.index') }}" class="sidebar-link active"><span>🏭</span> Suppliers</a>
    ...
    <main class="admin-main">
        ...
        <div class="form-card">
            <header class="admin-header" style="margin-bottom: 2rem; border-bottom: 1px solid #eee; padding-bottom: 1rem;">
                <h1 style="font-size: 1.5rem;">Edit Supplier</h1>
            </header>
            ...
            <form method="POST" action="{{ route('admin.suppliers.update', $supplier) }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="brand" value="{{ old('brand', $supplier->brand) }}" class="form-control" required placeholder="Enter brand name">
                </div>

                <div class="form-group">
                    <label class="form-label">Contact Person</label>
                    <input type="text" name="contact_person" value="{{ old('contact_person', $supplier->contact_person) }}" class="form-control" placeholder="Enter contact person name">
                </div>

                <div class="form-group">
                    <label class="form-label">Email</label>
                    <input type="email" name="email" value="{{ old('email', $supplier->email) }}" class="form-control" placeholder="Enter email address">
                </div>

                <div class="form-group">
                    <label class="form-label">Phone</label>
                    <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" class="form-control" placeholder="Enter phone number">
                </div>

                <div class="form-group">
                    <label class="form-label">Address</label>
                    <textarea name="address" rows="3" class="form-control" placeholder="Enter business address">{{ old('address', $supplier->address) }}</textarea>
                </div>
